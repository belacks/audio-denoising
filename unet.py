...
class UNet2d(nn.Module):
    ...
    def __init__(self, chnls_in=1, chnls_out=1, chnls_gs=32, dropout=0.01):
        ...
        self.dcl_6 = DownConvBlock2d(E,   F, kernel_size=3, stride=2, norm=False,           dropout=dropout)
        #self.dcl_7 = DownConvBlock2d(F,   G, kernel_size=(1,1), stride=(1,1), norm=False,           dropout=dropout)
        
        self.ucl_1 = UpConvBlock2d  (F,   E, kernel_size=3, stride=2, output_padding=(1,0),     dropout=dropout)
        ...
        #self.ucl_6 = UpConvBlock2d  (B+B, A, kernel_size=(2,2), stride=(2,2), output_padding=(0,1), dropout=dropout)
        self.ucl_0 =nn.ConvTranspose2d(A+A,O,kernel_size=3, stride=2, padding=1, output_padding=0)
        self.mlp = MLP(BINS, [512], BINS)
        #self.upsample_layer = nn.Upsample(scale_factor=2)
        #self.zero_pad = nn.ZeroPad1d(8)
        self.pre_mlp_act = nn.Tanh()
