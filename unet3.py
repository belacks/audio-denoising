...
class UNet2d3(nn.Module):
    ...
    def __init__(self, chnls_in=1, chnls_out=1, chnls_gs=32, dropout=0.01):
        ...
        self.dcl_1 = DownConvBlock2d(I+S, A, kernel_size=7,     stride=(5,3), dropout=dropout)
        self.dcl_2 = DownConvBlock2d(A,   B, kernel_size=5,     stride=3,     dropout=dropout)
        self.dcl_3 = DownConvBlock2d(B,   C, kernel_size=(9,3), stride=3,     dropout=dropout)
        
        self.ucl_1 = UpConvBlock2d    (C,   B, kernel_size=(9,3), stride=3,     output_padding=(0,1), dropout=dropout)
        self.ucl_2 = UpConvBlock2d    (B+B, A, kernel_size=5,     stride=3,     output_padding=(0,2), dropout=dropout)
        self.ucl_0 =nn.ConvTranspose2d(A+A, O, kernel_size=7,     stride=(5,3), output_padding=1,     padding=1)
    ...
    def forward(self, logmag):
        ...
        dec1 = self.ucl_1(enc3, enc2)
        #print("doing ucl_2", dec1.shape, enc4.shape)
        dec2 = self.ucl_2(dec1, enc1)
        #print("doing ucl_0", dec2.shape)
        dec0 = self.ucl_0(dec2)
